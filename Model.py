    ...
    def initialize(self):
        for module in self.modules():
            if isinstance(module, nn.Linear):
                init.xavier_uniform_(module.weight)  # uniform distribution
...
class AttnBlock(nn.Module):
    ...
    def __init__(self, in_ch, group):
        ...
        super().__init__()
        self.group_norm = nn.GroupNorm(group, in_ch)
...
class ResBlock(nn.Module):
    ...
    残差网络模块
    """
    def __init__(self, in_ch, out_ch, tdim, dropout, group, attn=False):
        """
        Args:
            in_ch: int, 输入通道数
            out_ch: int, 输出通道数
        ...
        if attn:
            self.attn = AttnBlock(out_ch, group)
        else:
...
class UNet(nn.Module):
    def __init__(self, in_channel, T, ch, ch_mult, attn, num_res_blocks, dropout):
        ...
        for i, mult in enumerate(ch_mult):  # i表示列表ch_mult的索引, mult表示ch_mult[i]
            ...
            for _ in range(num_res_blocks):
                self.downblocks.append(ResBlock(
                    ...
                    dropout=dropout, attn=(i in attn), group=ch))
                now_ch = out_ch
                chs.append(now_ch)
            if i != len(ch_mult) - 1:
        ...
        self.middleblocks = nn.ModuleList([
            ResBlock(now_ch, now_ch, tdim, dropout, attn=True, group=ch),
            ResBlock(now_ch, now_ch, tdim, dropout, attn=False, group=ch),
        ])
        ...
        self.tail = nn.Sequential(
            nn.GroupNorm(ch, now_ch),
            Swish(),
            nn.Conv2d(now_ch, in_channel, 3, stride=1, padding=1)
        )
...
class ResBlockSimple(nn.Module):
    ...
    残差网络模块
    """
    def __init__(self, in_ch, out_ch, dropout, group_num, attn=False):
        """
        Args:
            in_ch: int, 输入通道数
            out_ch: int, 输出通道数
...
class Encoder(nn.Module):
    def __init__(self, in_channel, out_channel, group_num, h_channels, num_res_blocks, attn, dropout=0.,
                 head=True, tail=True):
        super(Encoder, self).__init__()
        if isinstance(num_res_blocks, int):
            num_res_blocks = [num_res_blocks for _ in h_channels[1:]]

        if isinstance(attn, bool):
            attn = [attn for _ in h_channels[1:]]

        lst = [nn.Conv2d(in_channel, h_channels[0], 3, 1, 1)] if head else []

        now_channel = h_channels[0]

        for i, out_ch in enumerate(h_channels[1:]):
            num_block = num_res_blocks[i]
            att = attn[i]
            for _ in range(num_block):
                lst.append(ResBlockSimple(
                    now_channel, out_ch, dropout, group_num, att
                ))
                now_channel = out_ch
            lst.append(DownSample(now_channel))

        if tail:
            lst.append(nn.Conv2d(now_channel, out_channel, 1, 1, 0))
...
class Decoder(nn.Module):
    def __init__(self, in_channel, out_channel, group_num, h_channels, num_res_blocks, attn, dropout=0.,
        ...
        lst = [
            nn.Conv2d(in_channel, h_channels[0], 1, 1, 0),
        ] if head else []
        ...
        for i, out_ch in enumerate(h_channels[1:]):
            ...
            for _ in range(num_block):
                ...
                now_channel = out_ch
            lst.append(UpSample(now_channel))

        # lst.append(ResBlockSimple(now_channel, out_channel, dropout, 1, False))
        if tail:
            lst.append(nn.Conv2d(now_channel, now_channel, 3, 1, 1))
            lst.append(Swish())
            lst.append(nn.Conv2d(now_channel, out_channel, 1, 1, 0))
            lst.append(nn.Tanh())
